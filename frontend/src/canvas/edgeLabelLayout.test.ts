...
describe('shared edge label placement', () => {
  ...
  it('enumerates only rendered labels, including expanded bundle actions', () => {
    ...
    expect(keys('full')).toEqual(['relationship:r0', 'relationship:r1', 'relationship:r2', 'collapse'])
  ...
  it('reserves independent positions for work-state marks', () => {
    ...
    const expanded = placeEdgeLabels(graph.edges, graph.nodes, { detailLevel: 'full' }).get(edge.id)!
