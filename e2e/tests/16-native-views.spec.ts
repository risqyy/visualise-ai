...
test('16 · MCP views share identities and history while keeping local interaction and opaque deep links', async ({ page, request }, testInfo) => {
  ...
  try {
    ...
    await expect(canvas).toHaveAttribute('data-layouting', 'false')
    await page.getByTestId('canvas-toggle-tools').click()
    await expect(page.getByTestId('canvas-toggle-tools')).toHaveAttribute('aria-expanded', 'false')
    const box = await canvas.locator('.react-flow').boundingBox(); if (!box) throw new Error('canvas drawing surface missing')
